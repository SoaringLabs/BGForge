...
- 新 CD 进入同一阶段的任意副本时，按整张拍卖表检查旧掉落并清空，修复 P2、P3、P4、P5 合并表中另一副本的旧记录残留问题。
- 装备过滤会等待物品提示信息加载后再判断，修复职业限定兑换物及部分法系装备未正确置灰的问题。

### 优化

- 同一 CD 内切换到阶段中的另一副本，不会重复清空刚录入的拍卖记录。
- 装备过滤区域使用固定的简短标题，切换过滤方案时不再挤占按钮空间。
