...
do
    ...
    BG.scrollStep = 80
    BG.borderAlpha = .5
    -- BG.ver 保留上游协议版本，避免与 BGLite 玩家通信时被误判为旧版。
    -- BG.addonVer 才是本插件自身的发布版本。
    BG.ver = "v" .. (GetAddOnMetadata(AddonName, "X-Upstream-Version") or GetAddOnMetadata(AddonName, "Version"))
    BG.addonVer = "v" .. GetAddOnMetadata(AddonName, "Version")
    BG.BG = "|cff00BFFF<BGForge>|r "
    ...
    -- 但 BGForge 已删该自定义模板（Templates.xml），故直接写暴雪内置模板，功能等价、视觉为暴雪默认滚动条。
    ...
    if C_GameRules and C_GameRules.IsHardcoreActive and C_GameRules.IsHardcoreActive() then
        ...
        if not StaticPopupDialogs[frameName] then
            StaticPopupDialogs[frameName] = {
                text = L["BGForge插件提醒：由于暴雪对插件UI数量的限制，本插件无法在硬核服务器使用，请你自行禁用。"],
                button1 = L["好的"],
                OnAccept = function()
                end,
                whileDead = true,
...
do
    ...
    local Interface = "Interface\\AddOns\\BGForge\\Media\\sound\\"
        BG.soundAuthor = {
